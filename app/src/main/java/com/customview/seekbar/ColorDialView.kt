...
class ColorDialView @JvmOverloads constructor(
    ...
    init {
        val typedArray = context.obtainStyledAttributes(attributeSet, R.styleable.ColorDialView)
        try {
            colors = typedArray.getTextArray(R.styleable.ColorSlider_colors)
                .map {
                    Log.e("Log", "ColorDialView ${Color.parseColor(it.toString())}")
                    Color.parseColor(it.toString())
                } as ArrayList<Int>

            dialDiameter = typedArray
                .getDimension(R.styleable.ColorDialView_dialDiameter, toDP(100).toFloat()).toInt()
            ...
            scaleToFit = typedArray
                .getBoolean(R.styleable.ColorDialView_scaleToFit, false)
        } finally {
            typedArray.recycle()
        }
        dialDrawable = ContextCompat.getDrawable(context, R.drawable.ic_dial).also {
            it?.bounds = getCenteredBorder(dialDiameter)
            it?.setTint(Color.DKGRAY)
        }
        noColorDrawable = ContextCompat.getDrawable(context, R.drawable.ic_no_color).also {
            it?.bounds = getCenteredBorder(tickSize.toInt(), 2f)
        }
    ...
    private fun getCenteredBorder(size: Int, scalar: Float = 1f): Rect {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (scaleToFit) {
            ...
            dialDrawable?.let { it.bounds = getCenteredBorder((dialDiameter * scale).toInt()) }
            noColorDrawable?.let { it.bounds = getCenteredBorder((tickSize * scale).toInt(), 2f) }
